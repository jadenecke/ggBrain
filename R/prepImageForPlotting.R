  ...
  function(input,
    ...
    if (is.null(paletteRange)) {
      ...
      if (length(paletteRange) != 2) {
        stop("paletteRange must be of length 2")
      }
      if (paletteRange[1] > paletteRange[2]) {
        stop(
          "first value of paletteRange must be <= then the second value (first lower, then upper bound)"
        )
      }
    }
    #clamp input range
    ...
    df <- data.frame(
      x = rep(seq(nrow(input)), ncol(input)),
